...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      if(flag=1 && result.rows.length >0){
        res.render('esta', { name: result.rows[0].user_fname });
      }
    } else {
...
app.post('/esta', async (req, res) => {
    try {
        const rest= await pool.query(
            'SELECT FROM Establishment Where src_type=R',[restaurant1]
        );
    }catch (err) {
        console.error(err);
        res.send('Error logging in');
